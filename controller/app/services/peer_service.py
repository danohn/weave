...
from app.db.models import DestinationPolicy, Node, NodeStatus
...
async def get_peers(node: Node, session: AsyncSession) -> list[PeerResponse]:
    ...
    response = [
        PeerResponse(
            ...
            preferred_endpoint=(
                next(
                    (
                        ...
                        if link.is_active
                        and (link.reflected_endpoint_ip or link.endpoint_ip)
                    ),
                    None,
                )
            ...
            nat_detected=(peer.reflected_endpoint_ip or peer.endpoint_ip)
            != peer.endpoint_ip,
            site_subnet=peer.site_subnet,
            site_id=peer.site_id,
            site_name=peer.site.name if peer.site is not None else None,
            transport_link_id=next(
                (link.id for link in peer.transport_links if link.is_active), None
            ),
            transport_kind=next(
                (link.kind for link in peer.transport_links if link.is_active), None
            ),
        )
        for peer in peers
    ]
...
async def get_overlay_config(
    ...
    transports = sorted(
        [
            ...
            if link.admin_state_up and link.wireguard_public_key and link.overlay_vpn_ip
        ],
        ...
        for transport in transports:
            ...
            peers.append(
                PeerResponse(
                    ...
                    nat_detected=(remote.reflected_endpoint_ip or remote.endpoint_ip)
                    != remote.endpoint_ip,
        ...
        for transport in transports:
            peers.append(
                PeerResponse(
                    ...
                    wireguard_public_key=controller_pubkey,
                    vpn_ip=transport.controller_vpn_ip
                    or controller_overlay_ip_for_kind(transport.kind.value),
                    overlay_vpn_ip=transport.controller_vpn_ip
                    or controller_overlay_ip_for_kind(transport.kind.value),
                    preferred_endpoint=settings.WEAVE_DOMAIN,
                    endpoint_port=settings.CONTROLLER_ENDPOINT_PORT,
                    nat_detected=False,
    ...
    policies = [
        ...
        if policy_applies_to_node(item, node)
    ]
    ...
    return OverlayConfigResponse(
        transports=[
            OverlayTransportConfig(
                ...
                or controller_overlay_ip_for_kind(link.kind.value),
        ...
        destination_policies=[
            _resolve_destination_policy(node, policy) for policy in policies
        ],
    )
