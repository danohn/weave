    ...
    payload = {
        "nodes": [
            build_node_admin_response(
                n, bgp=bgp, policies=policies, events=event_map.get(n.id, [])
            ).model_dump(mode="json")
            for n in nodes
        ],
        "claims": [
            DeviceClaimResponse.model_validate(c).model_dump(mode="json")
            for c in claims
        ],
        "bgp": bgp,
        "events": [
            SiteEventResponse.model_validate(item).model_dump(mode="json")
            for item in events
        ],
        "policies": [
            {
                ...
                if getattr(p, "site", None) is not None
                else None,
                "node_id": p.node_id,
                "node_name": p.node.name
                if getattr(p, "node", None) is not None
                else None,
